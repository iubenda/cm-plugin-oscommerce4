<?php
/**
 * @copyright	Copyright (c) 2024 ConsentManager.net . All rights reserved.
...
final class consentmanager extends AbstractGoogle {

    public $config;
    public $code = 'consentmanager';

    public function getParams() {

        $this->config = [
            $this->code => [
                'name' => 'consentmanager CMP',
                'fields' => [
                    [
                        'name' => 'cmp_id',
                        'type' => 'text',
    ...
    public function renderWidget($example = false) {
        ...
        $cmp_id = $elements['fields'][0]['value'];
        $mode = $elements['type']['selected'];
        
        if ($mode == 'semi-automatic') {//semi-automatic
            $code = <<<EOD
            ...
            $code = <<<EOD
                <script type="text/javascript" data-cmp-ab="1" src="https://cdn.consentmanager.net/delivery/autoblocking/{$cmp_id}.js" data-cmp-host="delivery.consentmanager.net" data-cmp-cdn="cdn.consentmanager.net" data-cmp-codesrc="2"></script>
            EOD;
        }
    ...
    border-radius: 4px;">Consent Management Provider (CMP) helps your website can become GDPR & CCPA compliant with our Cookie Consent Solution
            <ul class="list">
            ...
            <a href="https://www.consentmanager.net/register/" target="_blank" style="text-decoration:underline"><b>Register Now</b></a></div>';
    }
}